...
```{r}
counts3 <- simulate_counts(biomass_k = 75000)
counts3 %>%
  ggplot(aes(date, extrapolated_biomass, color=factor(replicate))) + geom_point() +
  scale_y_log10() + my_theme +
  geom_smooth(methdod = "lm")
...
m3 %>% tidy(conf.int = TRUE)
```

```{r}
counts3b <- simulate_counts(biomass_k = 75000, dilute_at = 20000)
counts3b %>%
  ggplot(aes(date, extrapolated_biomass, color=factor(replicate))) + geom_point() +
  scale_y_log10() + my_theme +
  geom_smooth(methdod = "lm")
...
m3b %>% tidy(conf.int = TRUE)
```

Early dilution didn't work to solve the density dependence problem.
